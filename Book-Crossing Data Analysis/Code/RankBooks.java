...
public class RankBooks {
	...
	public static class BookJoinMap extends
		...
		public void map(LongWritable key, Text value, Context context)
			...
			if (!yr.equals("") && !arr[0].equals("ISBN")) {
				isbn = arr[0];
	...
	public static class BookRankJoinMap extends
			Mapper<LongWritable, Text, Text, Text> {
		public void map(LongWritable key, Text value, Context context)
				throws IOException, InterruptedException {
			String line = value.toString();
	...
	public static void main(String[] args) throws Exception {
		...
		joinJob.setJarByClass(RankBooks.class);
		...
		MultipleInputs.addInputPath(joinJob, new Path(args[1]),
				TextInputFormat.class, BookRankJoinMap.class);
		...
		analyzeJob.setJarByClass(RankBooks.class);
		...
		// Executing the program in a new thread.
		new Thread(jc).start();

		// Exiting only when all jobs in JobControl are done executing
